...
class PhenotypeBuilder : 
    ...
    def _query_cppn(self, coordinate1, coordinate2, one_towards_two, max_weight, output_node_idx, bias=1.0) :
        ...
        outputs = self.cppn.activate(inputs)
    ...
    def _connect_target_node_to_layer(self, target_node_coordinate, source_layer_coordinates, node_dict, one_towards_two, max_weight, output_node_idx) :
        ...
        for source_node_coordinate in source_layer_coordinates :
            weight = self._query_cppn(source_node_coordinate, target_node_coordinate, one_towards_two, max_weight, output_node_idx)
            if weight != 0.0 : 
                # here the dictionnary contains : key : coordinate of the node, value : index of the node
                incoming_connections.append((node_dict[tuple(source_node_coordinate)], weight))
        return incoming_connections

    def create_phenotype_network(self, substrate, activation_function="tanh", out_activation_function="identity", max_weight=1, output_node_idx=0) :
        ...
        for layer in all_layers[1:] : 
            ...
            for hidden_node in layer : 
                ...
                incoming_connections = self._connect_target_node_to_layer(hidden_node, source_layer, node_dict, one_towards_two, max_weight, output_node_idx)
